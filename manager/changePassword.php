...
$checkStaff = 'SELECT * FROM staff WHERE StaffID=?';
$stmt = $conn->prepare($checkStaff);
$stmt->bind_param('s', $id);
$stmt->execute();
$user = mysqli_stmt_get_result($stmt);
if ($row = mysqli_fetch_assoc($user)) {
    if (password_verify($oldPassword, $row['StaffPassword'])) {
        $_SESSION['verified'] = true;
    } else {
        echo ("<script>
                window.alert('Fail to verify your password.');
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="text-center">
                            <h1 class="text-capitalize mb-4 font-lg text-white">Change Password</h1>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="container">
                <div class="row">
                    <div class="col-md-14 col-sm-14 col-lg-12">
                        ...
                        <hr>
                        <form action="changePasswordManager.php" method="post" autocomplete="off">
                            <div>
                                <label for="password"><b>Password</b></label>
                                <div class="form-group">
                                    <input class="form-control" type="password" id="psw" name="AccountPassword" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" maxlength="20" placeholder="Password" title="Must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters" required>
                            ...
                            <div class="form-group">
                                ...
                                <div>
                                    <input class="form-control" type="password" id="psw2" name="AccountPassword2" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" maxlength="20" placeholder="Password" title="Must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters" required>
