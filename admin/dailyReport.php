...
if ($_POST['restaurantID'] == "all") {
    ...
    $sql = "SELECT *, SUM(TotalCost) AS Sales FROM orders NATURAL JOIN orderdetail NATURAL JOIN dish NATURAL JOIN restaurant WHERE OrderDate = date(NOW()) GROUP BY RestaurantID ";
    ...
    $sql = "SELECT *, SUM(TotalCost) AS Sales FROM orders NATURAL JOIN orderdetail NATURAL JOIN dish WHERE OrderDate = date(NOW()) AND RestaurantID = " . $_POST['restaurantID'] . " GROUP BY DishID ";
            ...
            <div class="container">
                ...
                <div id="table">
                    <table cellpadding="10" cellspacing="0" width="100%">
                        ...
                        <?php
                        if ($_POST['restaurantID'] == 'all') {
                            $sql2 = "SELECT *, SUM(TotalCost) AS Sales FROM orders NATURAL JOIN orderdetail NATURAL JOIN dish NATURAL JOIN restaurant WHERE OrderDate = date(NOW()) GROUP BY RestaurantID";
                        ...
                        ?>
                            ...
                            $sql2 = "SELECT *, SUM(TotalCost) AS Sales FROM orders NATURAL JOIN orderdetail NATURAL JOIN dish NATURAL JOIN restaurant WHERE OrderDate = date(NOW()) AND RestaurantID = " . $_POST['restaurantID'] . " GROUP BY DishID ";


                            $result = mysqli_query($conn, $sql2);
                            if ($result->num_rows > 0) {
                                while ($row = mysqli_fetch_assoc($result)) {
                                    $res_name = $row['DishName'];
                                    $dateorder = $row['OrderDate'];
                                    $sales = $row['Sales'];
                                ?>
                                    <tr>
